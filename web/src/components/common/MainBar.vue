<template>
  ...
  >
    ...
    <div v-else class="flex flex-shrink items-center text-xs md:text-base text-white mx-6 gap-4">
      <img 
        :src="this.$store.getters['api_host'] + user.avatar" 
      ...
      <div class="origin-top-right top-0 right-0.5 absolute profile-menu pt-16 group text-sm">
        <div class="bg-darkt-900 group-hover:border-b group-hover:border-l border-darkt-600 rounded-bl-lg flex flex-col hover:focus pl-5 pr-6">
          <button @click="console.log('Settings')" class="hover:text-blue-sapphire text-neutral-400 fill-neutral-400 hover:fill-blue-sapphire p-2 my-2">
            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" x="0px" y="0px" viewBox="0 0 486.4 486.4" xml:space="preserve" class="w-4 h-4 inline">
              <g>
          ...
          <div class="pt-2 pb-4">
            <button @click="logOut" class="hover:text-blue-sapphire text-neutral-400 fill-neutral-400 hover:fill-blue-sapphire p-2">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024" class="w-4 h-4 inline">
  ...
  <transition name="queuebar">
    <div v-if="show_queue_bar" class="absolute flex w-full h-10 bg-emerald-600 border-b border-t border-emerald-300 items-center shimmer" >
      <div class="flex-grow" />
